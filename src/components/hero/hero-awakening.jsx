...
class HeroAwakening extends Component {
    ...
    onBlur = (type, value, i, j) => {
        ...
        if (name[0] === "stats") {
            ...
            awakening[i]["statsIncrease"][j][type] = value % 1 === 0 ? parseInt(value) : parseFloat(value);
        } else {
                ...
                {awakening.map((increase, i) => (
                    <FormGroup key={i} className="inline-wrapper full">
                        {i !== 2 ? (
                            <React.Fragment>
                                <InputGroup>
                                    <InputGroupAddon addonType="prepend">{"Rank " + (i + 1)}</InputGroupAddon>
                                    <Input
                                        type="select"
                                        bsSize="sm"
                                        name={"stats." + Object.keys(increase["statsIncrease"][0])}
                                        value={Object.keys(increase["statsIncrease"][0])[0]}
                                        onBlur={e => this.onBlur(e.currentTarget.name, e.currentTarget.value, i, 0)}
                                        onChange={e =>
                                            this.handleChange(e.currentTarget.name, e.currentTarget.value, i, 0)
                                        }
                                    >
                                ...
                                <Input
                                    ...
                                    value={increase["statsIncrease"][0][Object.keys(increase["statsIncrease"][0])]}
                                    onBlur={e => this.onBlur(e.currentTarget.name, e.currentTarget.value, i, 0)}
                                    onChange={e => this.handleChange(e.currentTarget.name, e.currentTarget.value, i, 0)}
                                />
                            </React.Fragment>
                        ) : null}
                    </FormGroup>
